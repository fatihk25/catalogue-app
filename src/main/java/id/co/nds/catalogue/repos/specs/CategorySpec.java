...
public class CategorySpec implements Specification<CategoryEntity> {
    ...
    @Override
    public Predicate toPredicate(Root<CategoryEntity> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
        ...
        .equalsIgnoreCase(GlobalConstant.REC_STATUS_ACTIVE) || categoryModel.getRecStatus().trim().equalsIgnoreCase(GlobalConstant.REC_STATUS_NON_ACTIVE))) {
